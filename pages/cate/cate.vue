<template>
  <view class="scrollViewContainer">
    ...
    <scroll-view 
      ...
      class="leftScroll">
      <block
        v-for="(cateList,index) in scrollList"
        :key="inedx">
        <view 
          :class="['leftScrollItem', index === active ? 'active' : '']"
          @click="clickLeftItemHandler(index)"
    ...
    <scroll-view 
      ...
      <!-- 滚动区域二级分类 -->
      <view
        v-for="(rightItem, index) in scrollListLV2"
        :key="index">
          <!-- 滚动区域二级分类标题 -->
          ...
          <view class="rightItemImgLV3">
            <view 
              class="rightItem"
              v-for="(itemLV3, indexLV3) in rightItem.children"
              :key="indexLV3"
